...
uses Flink checkpoints to trigger idempotent DuckLake transactions
...
<table class="colwidths-auto docutils">
  ...
  <tbody>
    ...
    <tr><td>Rename column/table</td><td>Not supported</td><td>Route to a new target table instead.</td></tr>
...
and source primary-key metadata. The latter allows DDL validation to continue
after the metadata operator is restored or recreated.
...
Data changes and the checkpoint marker become visible atomically for a
successful checkpoint transaction. DDL is applied synchronously by the
metadata operator in a separate catalog transaction, so this boundary should
not be described as one transaction covering both DDL and DML.
