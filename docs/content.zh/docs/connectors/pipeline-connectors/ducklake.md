...
* **Checkpoint 驱动提交：**
由 Flink checkpoint 触发可幂等重试的 DuckLake 事务
* **Schema 同步：**
...
<table class="colwidths-auto docutils">
  ...
  <tbody>
    ...
    <tr><td>Rename column/table</td><td>不支持</td><td>请路由到新的目标表。</td></tr>
...
DuckLake 内部 schema `_flink_cdc_internal` 保存 checkpoint marker 和源表主键
元数据，使 metadata operator 在恢复或重建后仍能继续校验 DDL。
...
成功 checkpoint 事务中的数据变更和 marker 原子可见。DDL 由 metadata operator
在独立 catalog 事务中同步执行，因此不能将该边界描述为同时覆盖 DDL 和 DML 的单个事务。
