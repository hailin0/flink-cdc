...
final class DuckLakeDataFileImporter {
    ...
    void importFiles(List<DuckLakeCommittable> committables) throws SQLException {
        ...
        String keyTable = TEMPORARY_KEY_TABLE_PREFIX + first.getWriteResultHash().substring(0, 16);
    ...
    private SchemaFlags schemaFlags(DuckLakeWriteResult writeResult, String schema)
        ...
        List<String> targetCommonNames =
                targetNames.stream()
                        .filter(declaredByName::containsKey)
                        .collect(Collectors.toList());
        List<String> fileCommonNames =
                fileNames.stream().filter(targetNames::contains).collect(Collectors.toList());
        if (!targetCommonNames.equals(fileCommonNames)) {
            throw new SQLException(
                    "DuckLake target and Parquet common columns have different order");
        }
        boolean missingTargetColumnSeen = false;
        for (SchemaColumn targetColumn : target) {
            DuckLakeColumnMetadata declared = declaredByName.get(targetColumn.name);
            if (declared == null) {
                missingTargetColumnSeen = true;
            } else if (missingTargetColumnSeen) {
                throw new SQLException(
                        "Target columns missing from the Parquet file must be appended");
            }
