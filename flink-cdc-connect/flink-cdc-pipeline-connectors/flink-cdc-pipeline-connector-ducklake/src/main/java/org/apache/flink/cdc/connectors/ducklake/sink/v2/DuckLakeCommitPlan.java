...
public final class DuckLakeCommitPlan {
    ...
    private static final Comparator<DuckLakeCommittable> ORDERING =
            Comparator.comparing(DuckLakeCommittable::getTableId, TABLE_COMPARATOR)
                    ...
                    .thenComparingInt(DuckLakeCommitPlan::kindOrder)
                    .thenComparingLong(DuckLakeCommitPlan::dataFileSequence)
                    .thenComparingInt(DuckLakeCommittable::getSubtaskId)
                    .thenComparingInt(DuckLakeCommittable::getAttemptNumber);
    ...
    public static DuckLakeCommitPlan from(Collection<DuckLakeCommittable> input)
        ...
        for (DuckLakeCommittable current : sorted) {
            if (!deduplicated.isEmpty()) {
                ...
                if (hasSameLogicalKey(previous, current)) {
                    if (!previous.getPayloadHash().equals(current.getPayloadHash())) {
    ...
    private static String hash(List<DuckLakeCommittable> committables) throws IOException {
        try {
            DataOutputSerializer canonical = new DataOutputSerializer(512);
            canonical.writeInt(1);
            DuckLakeCommittable first = committables.get(0);
            DuckLakeCommittableSerializer.writeString(canonical, first.getSinkId());
            DuckLakeCommittableSerializer.writeString(canonical, first.getOperatorId());
            canonical.writeLong(first.getCheckpointId());
            for (DuckLakeCommittable committable : committables) {
                canonical.writeBoolean(committable.isSchemaChange());
                TableIdSerializer.INSTANCE.serialize(committable.getTableId(), canonical);
                canonical.writeInt(committable.getSchemaBatchIndex());
                if (!committable.isSchemaChange()) {
                    canonical.writeLong(committable.getFileSequence());
                    canonical.writeInt(committable.getSubtaskId());
                }
                DuckLakeCommittableSerializer.writeString(canonical, committable.getPayloadHash());
            }
            return DuckLakeHashUtils.toHex(
                    MessageDigest.getInstance("SHA-256").digest(canonical.getCopyOfBuffer()));
        } catch (NoSuchAlgorithmException e) {
            throw new IOException("SHA-256 is not available", e);
        }
    }

    private static int kindOrder(DuckLakeCommittable committable) {
        return committable.isSchemaChange() ? 0 : 1;
    }

    private static long dataFileSequence(DuckLakeCommittable committable) {
        return committable.isSchemaChange() ? 0 : committable.getFileSequence();
    }

    private static boolean hasSameLogicalKey(
            DuckLakeCommittable first, DuckLakeCommittable second) {
        if (!first.getTableId().equals(second.getTableId())
                || first.getSchemaBatchIndex() != second.getSchemaBatchIndex()
                || first.isSchemaChange() != second.isSchemaChange()) {
            return false;
        }
        return first.isSchemaChange()
                || (first.getFileSequence() == second.getFileSequence()
                        && first.getSubtaskId() == second.getSubtaskId());
    }
}
