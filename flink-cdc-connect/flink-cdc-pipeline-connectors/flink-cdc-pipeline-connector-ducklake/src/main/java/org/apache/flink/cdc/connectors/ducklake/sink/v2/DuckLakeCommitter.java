...
public final class DuckLakeCommitter implements Committer<DuckLakeCommittable> {
    ...
    private final AtomicInteger pendingCommittables = new AtomicInteger();
    ...
    DuckLakeCommitter(
        ...
        if (metricGroup != null) {
            metricGroup.setCurrentPendingCommittablesGauge(pendingCommittables::get);
        }
    }
    ...
    public void commit(Collection<CommitRequest<DuckLakeCommittable>> requests) throws IOException {
        ...
        pendingCommittables.set(requests.size());
        try {
            ...
            for (List<CommitRequest<DuckLakeCommittable>> checkpointRequests :
                    byCheckpoint.values()) {
                try {
                    ...
                    cleanupOrphansIfDue();
                    pendingCommittables.addAndGet(-checkpointRequests.size());
    ...
    private boolean commitPlan(DuckLakeCommitPlan plan) throws IOException {
        for (int attempt = 0; attempt <= maxRetries; attempt++) {
            ...
            try {
                ...
                for (List<DuckLakeCommittable> batch : batches(plan.getCommittables())) {
    ...
    private static List<List<DuckLakeCommittable>> batches(List<DuckLakeCommittable> committables) {
        List<List<DuckLakeCommittable>> batches = new ArrayList<>();
        for (DuckLakeCommittable committable : committables) {
            if (batches.isEmpty()
                    || !sameBatch(batches.get(batches.size() - 1).get(0), committable)) {
                batches.add(new ArrayList<>());
            }
            batches.get(batches.size() - 1).add(committable);
        }
        return batches;
    }

    private static boolean sameBatch(DuckLakeCommittable left, DuckLakeCommittable right) {
        return left.getTableId().equals(right.getTableId())
